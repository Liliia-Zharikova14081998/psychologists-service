...
import css from "./PsychologistsView.module.css"; 
import PsychologistCard from '../PsychologistCard/PsychologistCard';

const options: { value: SortOption; label: string }[] = [
  { value: 'asc', label: 'A to Z' },
  { value: 'desc', label: 'Z to A' },
  { value: 'price_low', label: 'Less than 10$' },
  { value: 'price_high', label: 'Greater than 10$' },
...
const PsychologistsView = () => {
  ...
  const sortedPsychologists = [...allPsychologists].sort((a, b) => {
      ...
      <main className={css.main}>
        {isLoading ? (
          <div className={css.loader}>
            <PuffLoader color="#54be96" size={80} />
          </div>
        ) : (
          <>
            <div className={css.list}>
              {visibleItems.map((psychologist) => (
                <div key={psychologist.id || psychologist.name} className={css.cardPlaceholder}>
